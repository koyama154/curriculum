...
SELECT st.store_name, gt.goods_name, sct.quantity
FROM stock_table sct
JOIN store_table st ON sct.store_code = st.store_code
JOIN goods_table gt ON sct.goods_code = gt.goods_code;


-- 4. 商品テーブルから全商品の単価の平均値を抽出しなさい。
SELECT AVG(price) AS average_price FROM goods_table;
